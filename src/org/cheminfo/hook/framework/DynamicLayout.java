...
public class DynamicLayout implements LayoutManager, java.io.Serializable
{
	...
	public DynamicLayout(int stackDirection)
	{
		this(stackDirection, 0, 0);
	}
	
	public DynamicLayout(int stackDirection, int hGap, int vGap)
	{
	...
	public void layoutContainer(Container parent)
	{
		...
		int maxHeight=0;
		int maxWidth=0;
		ImageButton currentComponent=null;
		if (this.stackDirection == this.HORIZONTAL)
		{
			for (int comp=0; comp < parent.getComponentCount(); comp++)
			{
				currentComponent=(ImageButton)parent.getComponent(comp);
				if (currentComponent.getStatus() != ImageButton.INACTIVE)
				{
				...
				{
					currentComponent.setLocation(parent.getPreferredSize().width, lastYPos);
				}
				if (currentComponent.getPreferredSize().height > maxHeight) maxHeight=currentComponent.getPreferredSize().height;
			}
		}
		else 
		{
			for (int comp=0; comp < parent.getComponentCount(); comp++)
			{
				...
				{
					currentComponent.setLocation(lastXPos, parent.getPreferredSize().height);
				}					
				if (currentComponent.getPreferredSize().width > maxWidth) maxWidth=currentComponent.getPreferredSize().width;
			}
		}
