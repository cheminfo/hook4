...
public class ShowPredictionLabelActionButton extends DefaultActionButton {
	
	public ShowPredictionLabelActionButton(Image inImage, String infoMessage, InteractiveSurface interactions) {
	...
	protected void performInstantAction() {
		...
		if (interactions.getActiveEntity() instanceof Spectra)
			...
			tempSpectra = (Spectra) interactions.getActiveEntity()
					.getParentEntity();
		if (tempSpectra != null) {
			tempSpectra.switchVisiblePredictionLabels();
	...
	protected void checkButtonStatus() {
		...
		if (interactions.getActiveDisplay() instanceof SpectraDisplay
				&& tempSpectra != null) {
			this.activate();
			if (tempSpectra.hasVisiblePredictionLabels() == 1)
